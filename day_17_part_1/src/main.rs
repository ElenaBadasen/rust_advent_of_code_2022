...
fn clashes(tile: &Coordinate, field: &[[Terrain; 7]], offset: i64) -> bool {
...
fn move_rock(
	...
	let potential_coordinate: Coordinate = match direction {
		Direction::Left => {
			Coordinate{x: start_coordinate.x - 1, y: start_coordinate.y}
		},
		Direction::Right => {
			Coordinate{x: start_coordinate.x + 1, y: start_coordinate.y}
		},
		Direction::Down => {
			if start_coordinate.y == 0 {
				return (Coordinate{x: start_coordinate.x, y: start_coordinate.y}, true);
			}
			Coordinate{x: start_coordinate.x, y: start_coordinate.y - 1}	
		},
	};
	...
	for tile in rock_tiles(rock_type, &potential_coordinate) {
		if clashes(&tile, field, offset) {
			...
			return (Coordinate{x: start_coordinate.x, y: start_coordinate.y}, result);
		}
	}
	(Coordinate{x: potential_coordinate.x as i16, y: potential_coordinate.y as i64}, false)	
}
	...
	match rock_type {
		RockType::Horizontal => {
			vec![
				...
				Coordinate{x: coordinate.x + 2, y: coordinate.y},
				Coordinate{x: coordinate.x + 3, y: coordinate.y},
		...
		RockType::L => {
			vec![
				Coordinate{x: coordinate.x + 2, y: coordinate.y},
				Coordinate{x: coordinate.x + 2, y: coordinate.y - 1},
				Coordinate{x: coordinate.x + 2, y: coordinate.y - 2},
				Coordinate{x: coordinate.x, y: coordinate.y - 2},
				Coordinate{x: coordinate.x + 1, y: coordinate.y - 2},
			].into_iter().collect::<HashSet<Coordinate>>()
		},
		RockType::Vertical => {
			vec![
				...
				Coordinate{x: coordinate.x, y: coordinate.y - 1},
				Coordinate{x: coordinate.x, y: coordinate.y - 2},
				Coordinate{x: coordinate.x, y: coordinate.y - 3},
			].into_iter().collect::<HashSet<Coordinate>>()
		},
		RockType::Square => {
			vec![
...
fn main() {
	...
	let mut field: Vec<[Terrain; 7]> = vec![];
	for _i in 0..4 {
		field.push([Terrain::Air; 7]);
	}
	...
	while !end_reached {
		for rule in &rules {
			...
			let (new_coordinate, _) = move_rock(
				&rock_coordinate, 
				&rock_type, 
				&direction, 
				&field,
			...
			if rock_rested {
				...
				for tile in rock_tiles(&rock_type, &Coordinate{x: rock_coordinate.x, y: rock_coordinate.y}) {
				...
				if max_split_at_candidate > 0 {
					field.drain(0..(max_split_at_candidate as usize - offset as usize));
				...
				rock_coordinate = Coordinate{x: 2, y: highest_rock_level + 4 + rock_extra_height(&rock_type) as i64};
			}
		}
	}
