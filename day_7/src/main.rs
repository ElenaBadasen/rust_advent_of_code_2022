...
impl Dir {
	fn new(name: String, parent_dir: Option<Weak<Box<dyn Sizable>>>, children: Vec<Rc<Box<dyn Sizable>>>) -> Dir {
		Dir {
			name,
...
impl Sizable for Dir {
	...
	fn parent(&self) -> Option<Rc<Box<dyn Sizable>>> {
		Some(self.parent_dir.as_ref()?.upgrade()?)
	}
            ...
            if command_vec[0] == "$" {
				...
				if ls_on {
					if command_vec[0] == "dir" {
						let dir = Dir::new(command_vec[1].to_string(), Some(Rc::downgrade(&fs.current_dir)), vec![]);
						let rc_link: Rc<Box<dyn Sizable>> = Rc::new(Box::new(dir));
						fs.current_dir.children().unwrap().borrow_mut().push(rc_link.clone());
						...
						fs.current_dir.children().unwrap().borrow_mut().push(Rc::new(Box::new(file)));
					}
				} else {
					println!("Wrong input!");
				}
